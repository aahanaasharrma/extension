...
// Listen for messages from content script (content.js)
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "checkPageBlocked") {
    const urlToCheck = message.url;
    const currentHostname = new URL(urlToCheck).hostname.toLowerCase();
    isWebsiteBlocked(currentHostname, (isBlocked) => {
      if (isBlocked || isURLBlocked(urlToCheck)) {
  ...
  } else if (message.action === "addBlockedWebsite") {
